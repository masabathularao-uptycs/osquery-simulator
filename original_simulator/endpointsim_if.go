	...
	for _, data := range jsonMap.Data {
		...
		if rowData, ok := data["columns"]; ok {
			...
			if columnData, ok := rowData.(map[string]interface{}); ok {
				//log.Println("checking for system_id data")
				if _, exists := columnData["mnt_namespace"]; exists {
					//log.Println("system_id exists in data")
					//log.Println(newstr)
					if index<16{
						columnData["mnt_namespace"] ="mnt_namespace"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)+name
					}else{
						columnData["mnt_namespace"] ="mnt_namespace"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)
					}
				}
				if _, exists := columnData["name"]; exists {
                                        //log.Println("system_id exists in data")
                                        //log.Println(newstr)
										rand.Seed(time.Now().UnixNano())
										if index<16 {
                                        	columnData["name"] ="name"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)+name
										}else {
											columnData["name"] ="name"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)
										}
                                }
				if _, exists := columnData["mnt_namespace"]; exists {
										rand.Seed(time.Now().UnixNano())
				...
				if _, exists := columnData["image_name"]; exists {
                                        //log.Println("system_id exists in data")
                                        //log.Println(newstr)
										rand.Seed(time.Now().UnixNano())
										if index<16{
                                        	columnData["image_name"] ="image_name"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)+name
										}else{
											columnData["image_name"] ="image_name"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)
										}

                                }
				if _, exists := columnData["local"]; exists {
                                        //log.Println("system_id exists in data")
                                        //log.Println(newstr)
										rand.Seed(time.Now().UnixNano())
										if index<16{
                                        	columnData["local"] ="local"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)+name
										}else{
											columnData["local"] ="local"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)
										}

                                }
				if _, exists := columnData["local_address"]; exists {
                                        //log.Println("system_id exists in data")
                                        //log.Println(newstr)
										rand.Seed(time.Now().UnixNano())
										if index<16{
                                        	columnData["local_address"] ="local_address"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)+name
										}else{
											columnData["local_address"] ="local_address"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)
										}

                                }
				if _, exists := columnData["net_namespace"]; exists {
                                        //log.Println("system_id exists in data")
                                        //log.Println(newstr)
										rand.Seed(time.Now().UnixNano())
										if index<16{
                                        	columnData["net_namespace"] ="net_namespace"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)+name
										}else{
											columnData["net_namespace"] ="net_namespace"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)
										}
				...
				if _, exists := columnData["login_name"]; exists {
										...
										rand.Seed(time.Now().UnixNano())
										if index<16{
                                        	columnData["login_name"] ="login_name"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)+name
										}else{
											columnData["login_name"] ="login_name"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)
										}

                                }
				if _, exists := columnData["partial"]; exists {
                                        //log.Println("system_id exists in data")
                                        //log.Println(newstr)
										rand.Seed(time.Now().UnixNano())
										if index<16{
                                        	columnData["partial"] ="partial"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)+name
										}else{
											columnData["partial"] ="partial"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)
										}

                                }
				if _, exists := columnData["overflows"]; exists {
                                        //log.Println("system_id exists in data")
                                        //log.Println(newstr)
										rand.Seed(time.Now().UnixNano())
										if index<16{
                                        	columnData["overflows"] ="overflows"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)+name
										}else{
											columnData["overflows"] ="overflows"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)
										}

                                }
				if _, exists := columnData["ancestor_list"]; exists {
                                        //log.Println("system_id exists in data")
                                        //log.Println(newstr)
										rand.Seed(time.Now().UnixNano())
										if index<16{
                                        	columnData["ancestor_list"] ="ancestor_list"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)+name
										}else{
											columnData["ancestor_list"] ="ancestor_list"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)
										}

                                }
				if _, exists := columnData["socket"]; exists {
                                        //log.Println("system_id exists in data")
                                        //log.Println(newstr)
										rand.Seed(time.Now().UnixNano())
										if index<16{
                                        	columnData["socket"] ="socket"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)+name
										}else{
											columnData["socket"] ="socket"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)
										}

                                }
				if _, exists := columnData["integrity_level"]; exists {
                                        //log.Println("system_id exists in data")
                                        //log.Println(newstr)
										rand.Seed(time.Now().UnixNano())
										if index<16{
                                        	columnData["integrity_level"] ="integrity_level"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)+name
										}else{
											columnData["integrity_level"] ="integrity_level"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)
										}

                                }
				if _, exists := columnData["decision"]; exists {
                                        //log.Println("system_id exists in data")
                                        //log.Println(newstr)
										rand.Seed(time.Now().UnixNano())
										if index<16{
                                        	columnData["decision"] ="decision"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)+name
										}else{
											columnData["decision"] ="decision"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)
										}

                                }
				if _, exists := columnData["state"]; exists {
                                        //log.Println("system_id exists in data")
                                        //log.Println(newstr)
										rand.Seed(time.Now().UnixNano())
										if index<16{
                                        	columnData["state"] ="state"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)+name
										}else{
											columnData["state"] ="state"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)
										}

                                }
				if _, exists := columnData["remote"]; exists {
                                        //log.Println("system_id exists in data")
                                        //log.Println(newstr)
										rand.Seed(time.Now().UnixNano())
										if index<16{
                                        	columnData["remote"] ="remote"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)+name
										}else{
											columnData["remote"] ="remote"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)
										}

                                }
				if _, exists := columnData["mode"]; exists {
                                        //log.Println("system_id exists in data")
                                        //log.Println(newstr)
										rand.Seed(time.Now().UnixNano())
										if index<16{
                                        	columnData["mode"] ="mode"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)+name
										}else{
											columnData["mode"] ="mode"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)
										}

                                }
				if _, exists := columnData["partner_mode"]; exists {
                                        //log.Println("system_id exists in data")
                                        //log.Println(newstr)
										rand.Seed(time.Now().UnixNano())
										if index<16{
                                        	columnData["partner_mode"] ="partner_mode"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)+name
										}else{
											columnData["partner_mode"] ="partner_mode"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)
										}

                                }
				if _, exists := columnData["model_id"]; exists {
                                        //log.Println("system_id exists in data")
                                        //log.Println(newstr)
										rand.Seed(time.Now().UnixNano())
										if index<16{
                                        	columnData["model_id"] ="model_id"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)+name
										}else{
											columnData["model_id"] ="model_id"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)
										}

                                }
				if _, exists := columnData["pci_subclass_id"]; exists {
                                        //log.Println("system_id exists in data")
                                        //log.Println(newstr)
										rand.Seed(time.Now().UnixNano())
										if index<16{
                                        	columnData["pci_subclass_id"] ="pci_subclass_id"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)+name
										}else{
											columnData["pci_subclass_id"] ="pci_subclass_id"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)
										}

                                }
				if _, exists := columnData["vendor"]; exists {
                                        //log.Println("system_id exists in data")
                                        //log.Println(newstr)
										rand.Seed(time.Now().UnixNano())
										if index<16{
                                        	columnData["vendor"] ="vendor"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)+name
										}else{
											columnData["vendor"] ="vendor"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)
										}

                                }
				if _, exists := columnData["transport"]; exists {
									//log.Println("system_id exists in data")
									//log.Println(newstr)
									rand.Seed(time.Now().UnixNano())
									if index<16{
										columnData["transport"] ="transport"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)+name
									}else{
										columnData["transport"] ="transport"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)
									}

							    }
				if _, exists := columnData["reason"]; exists {
									//log.Println("system_id exists in data")
									//log.Println(newstr)
									rand.Seed(time.Now().UnixNano())
									if index<16{
										columnData["reason"] ="reason"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)+name
									}else{
										columnData["reason"] ="reason"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)
									}

							    }
				if _, exists := columnData["period"]; exists {
									//log.Println("system_id exists in data")
									//log.Println(newstr)
									rand.Seed(time.Now().UnixNano())
									if index<16{
										columnData["period"] ="period"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)+name
									}else{
										columnData["period"] ="period"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)
									}

								}
				if _, exists := columnData["average"]; exists {
									//log.Println("system_id exists in data")
									//log.Println(newstr)
									rand.Seed(time.Now().UnixNano())
									if index<16{
										columnData["average"] ="average"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)+name
									}else{
										columnData["average"] ="average"+strconv.Itoa(rand.Intn(max*(index+1)-min*(index))+min*index)
									}
