...
def analyse(line2):
        ...
        for key,value in pydict.items():
            ...
            if key == "tables":
                for table in list(pydict['tables'].keys()):
                    for record in pydict['tables'][table]:
                         if datastats_action.get(table) == None:
                             datastats_action[table]={"added":0,"removed":0,"snapshot":0}
                         if record.get('action') == "added":
                             datastats_action[table]["added"] = datastats_action[table]["added"] + 1
                         if record.get('action') == "removed":
                             datastats_action[table]["removed"] = datastats_action[table]["removed"] + 1
                         if record.get('action') == 'snapshot':
                             datastats_action[table]["snapshot"] = datastats_action[table]["snapshot"] + 1
                         if record.get('action') == None:
                             print("Warning ,name is missing in :",record)
                         #else:
                         #    datastats[table]=datastats[table]+1
...
def SendTrigger(sockudp,msg,destip,portlist,timefornexttrigger):
    ...
    for Port in portlist:
      thread.start_new_thread(actual_send, (msg,Port))
    ...
    with open(loadinput['inputfile']) as fs:
      ...
      while 1:
        ...
        if abs(diff_ts) > 6:
          ...
          diff_ts=4.0
        thread.start_new_thread(SendTrigger, (sock,con,UDP_IP,portlist[0:trafficinstances],diff_ts))
        time.sleep(diff_ts)
        #time.sleep(3)
        first_ts=second_ts
        if endline != 0:
